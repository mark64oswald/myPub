...
myPub transforms a collection of ~570 technical ePub books into an intelligent, queryable knowledge base for Claude. The v1 design preserves document structure, captures concept relationships manually, and loads full chapters into Claude's context rather than chunking them into embeddings.
...
- **Manual concept curation doesn't scale** to ~570 books.
...
This project is a knowledge base system indexing ~570 technical ePubs,
...
The server opens the local DuckDB on startup, loads the FTS, VSS, and DuckPGQ extensions, and warms caches. FTS and VSS indexes may need to be rebuilt on startup if HNSW persistence is unstable — for ~570 books this is a minutes-not-hours operation.
...
**HNSW persistence.** Still experimental in DuckDB. If unstable, fall back to in-memory indexes rebuilt on MCP server startup. For ~570 books this is minutes, acceptable for development; revisit if cold start becomes annoying.
